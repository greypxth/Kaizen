...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Kaizen, Tony Stark's AI assistant. "
...
def _update_memory_async(user_text: str, Kaizen_text: str) -> None:
    """
...
class KaizenLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[Kaizen] 🔧 TOOL: {name}  ARGS: {args}")
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            print(f"[Kaizen] ❌ Mic error: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[Kaizen] 👂 Recv started")
        out_buf = []
        in_buf  = []
        ...
        try:
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if out_buf:
                                ...
                                if full_out:
                                    self.ui.write_log(f"Kaizen: {full_out}")
                            out_buf = []
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[Kaizen] 📞 Tool call: {fc.name}")
            ...
            print(f"[Kaizen] ❌ Recv error: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[Kaizen] 🔊 Play started")
        stream = await asyncio.to_thread(
            pya.open,
            format=FORMAT,
            channels=CHANNELS,
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[Kaizen] ✅ Connected.")
                    self.ui.write_log("Kaizen online.")
                ...
                print(f"[Kaizen] ⚠️  Error: {e}")
                traceback.print_exc()

            print("[Kaizen] 🔄 Reconnecting in 3s...")
            await asyncio.sleep(3)

def main():
    ui = KaizenUI("face.png")

    def runner():
        ui.wait_for_api_key()
        
        Kaizen = KaizenLive(ui)
        try:
            asyncio.run(Kaizen.run())
